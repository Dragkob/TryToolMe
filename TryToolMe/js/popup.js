...
function setCapabilityScoreDisplay(rawScore) {
  ...
  if (typeof rawScore === "string" && rawScore.trim()) {
    txt = rawScore.trim();
  } else if (typeof rawScore === "number" && isFinite(rawScore)) {
    txt = String(rawScore);
  }
...
function renderCapabilityScoreDetail(apiBody) {
  ...
  var raw = cs.rawScore;
  var rawStr = raw != null && raw !== "" ? escapeHtmlCap(String(raw)) : "—";
  ...
  if (hist.length) {
    ...
    for (var hi = 0; hi < hist.length && hi < maxHist; hi++) {
      ...
      var hScore = h.rawScore != null && h.rawScore !== "" ? escapeHtmlCap(String(h.rawScore)) : "—";
      var hc = h.components || {};
      var mini = "";
